...
all_coords <- rbind(spat_coords, filter(temp_coords, site_id=="J07"))
...
full_area1 <- GOAmap1 + coord_map(projection="gilbert") +
  geom_point(data = all_coords, aes(x = long, y = lat), size = 3,# shape=21#,
             shape=c(13, 1, 1, 1, 1, 1, 4)
             #fill=c("darkred", "#E41A1C", "#F781BF", "#053061", "lightseagreen", "#1F78B4")
             )+
  geom_text(data = all_coords, aes(x = long, y = lat, label = site_id), hjust=0.25, vjust=-1, fontface = "bold") +
  #geom_label_repel
  #geom_ellipse(aes(x0 = -126.28, y0 = 50.47, a = 0.1, b = 0.66, angle = pi/2.27), size = 0.8, color = "grey30") +
  #geom_ellipse(aes(x0 = -125.1, y0 = 50.23, a = 0.45, b = 0.275, angle = 0), size = 0.8, color = "grey30") +
  theme_bw() +
  ...
  annotate("text", x = -125.25, y = 50.9, label = "Mainland\nBritish\nColumbia", color = "grey30", size = 5)
full_area1
ggsave("full_area_basic.png", path = "./figs/map", width = 16, height = 12, units = "cm", dpi = 300)

png(filename = "./figs/map/full_map.png", width = 7, height = 5, units = "in", res = 300)#, type = "cairo")
